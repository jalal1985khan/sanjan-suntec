...
const SinglePost = ({tags}) => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const postResponse = await fetch(`${configData.SERVER_URL}ebooks/${tags}`);
  ...
  return (
    <>
      <h2 className='text-center mb-5'>ebooks</h2>
    ...
    <div class="col-md-8 text-start d-flex align-items-center">
      <div class="card-body">
        <h5 class="card-title text-start" dangerouslySetInnerHTML={{__html:post.title.rendered}}/>
                      ...
                      <Link href={`ebooks/${post.slug}`} className="pr-text text-decoration-none expand-btn text-white px-5 rounded-3">Download Now</Link>
                    </div>
